...
struct SumOp {
    ...
    static __host__ __device__ constexpr T identity() { return T(0); }
};

template <typename T>
struct MaxOp {
    __device__ __forceinline__ T operator()(T a, T b) const { return max(a, b); }
    static __host__ __device__ constexpr T identity() { return -INFINITY; }  // For float
};
...
struct ProdOp {
    __device__ __forceinline__ T operator()(T a, T b) const { return a * b; }
    static __host__ __device__ constexpr T identity() { return T(1); }
};
...
std::size_t rand_state_bytes(int batch_size) {
    return batch_size > 0 ? static_cast<std::size_t>(batch_size) * sizeof(RAND) : 0;
}
...
cudaError_t setup_rand_raw(
    ...
    setup_rand_kernel<<<batch_size, 1, 0, stream>>>(
